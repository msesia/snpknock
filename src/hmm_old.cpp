...
#ifndef O_KNOCKOFF_HMM_CPP
#define O_KNOCKOFF_HMM_CPP

#include "hmm_old.h"

oKnockoffHMM::oKnockoffHMM(const std::vector<double> & _initP, const std::vector< matrix > & _Q, \
                         const std::vector< matrix > & _emissionP, int seed) {
  initP = _initP;
  Q = _Q;
  emissionP = _emissionP;
  H_knockoffs = new oKnockoffDMC(initP, Q, seed+100000);
  nStates = initP.size();
...
oKnockoffHMM::~oKnockoffHMM(){
  delete H_knockoffs;
}

std::vector<int> oKnockoffHMM::sample(const std::vector<int> & X) {
...
std::vector< std::vector<int> > oKnockoffHMM::sample(const std::vector<std::vector<int> > & X) {
  int n = X.size();
...
void oKnockoffHMM::sampleHMMConditional(const std::vector<int> & X) {
...
void oKnockoffHMM::backwardHMM(const std::vector<int> & X) {
  std::fill(beta[p-1].begin(), beta[p-1].end(), 1.0);
